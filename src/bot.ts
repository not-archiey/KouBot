...
(async () => {
  // Load command files
  const commandsFiles = readdirSync(path.join(__dirname, 'commands')).filter((file) => file.endsWith('.js'));

  // Load commands
  for (const file of commandsFiles) {
    const { default: ctor } = await import(path.join(__dirname, 'commands', file));

    if (!ctor) continue;

    // Checks if we actually have a command here
    // we don't want to register classes that are not commands
...
taiga.on('guildMemberAdd', (member) => {
  ...
  for (const id of generalChannelIds) {
    ...
    const { greetings } = enStrings.texts;
...
taiga.on('message', async (message) => {
  ...
  let config = MEMBER_CONFIG.find((cfg) => cfg.userId === message.author.id);
  if (config) {
    console.log(config);
    console.log('Found config.');
  } else {
    MEMBER_CONFIG.push({ user: message.author, userId: message.author.id, lang: 'en' });
    config = MEMBER_CONFIG.find((cfg) => cfg.userId === message.author.id);
