...
export default class ClientUtility {
  ...
  static randomUserReactionHandler(message: Discord.Message) {
    ...
    if (hitMiss) {
      if (message.author.id === '180740743852326912') { message.reply(RHAKON_REACTIONS[getRandomInt(0, RHAKON_REACTIONS.length)]); } else if (message.author.id === '215526684797960192') { message.reply(KACHI_REACTIONS[getRandomInt(0, KACHI_REACTIONS.length)]); } else if (message.author.id === '169936831373115393') { message.reply(MAGIC_REACTIONS[getRandomInt(0, MAGIC_REACTIONS.length)]); } else if (message.author.id === '263348633280315395') { message.reply(BRANDON_REACTIONS[getRandomInt(0, BRANDON_REACTIONS.length)]); }
    }
  }
