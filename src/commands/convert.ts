...
export default class Convert extends Command {
  ...
  private convert(targetUnit: string, input: string, message: Discord.Message) {
    ...
    if (Number.isNaN(numberToConvert)) { return errorMsgs.is_nan; }
    ...
    for (const type of tables) {
      ...
      switch (targetUnit) {
        case 'c':
          if (sourceUnit === 'f') { numberToConvert -= 32; } else if (sourceUnit === 'k') { numberToConvert -= 273.15; }
          result = type[targetUnit][sourceUnit] * numberToConvert;
          break;
        case 'f':
          result = type[targetUnit][sourceUnit] * numberToConvert;
          if (sourceUnit === 'c') { result += 32; } else if (sourceUnit === 'k') { result -= 459.67; }
          break;
        case 'k':
          if (sourceUnit === 'c') { numberToConvert += 273.15; } else if (sourceUnit === 'f') {
